...
{
	// Use Timer0 ISR instead of timer task
	// Setup Comparison register OCR0A ( timer/counter 0 output compare register A )
	// To call an interrupt whenever (TCNT0 matches OCR0A), set the TIMSK0 : OCIE0A bit,
	// and write a handler for the TIMER0_COMPA interrupt vector.

	  OCR0A = 0xAF;
	  TIMSK0 |= _BV(OCIE0A);
	  Serial.println(F("Set up Timer0 for POS"));
...
/************************************************************************
* Function	: ISR(TIMER0_COMP_vect)
* Description	: Arduino Interrup Service routine for OCR0A
************************************************************************/
