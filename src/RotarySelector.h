...
#define ROTDBAMOUNT 5					// 5ms for rotary debounce
...
public:
	...
	virtual void rotaryAction(const int type, int counterOrLevel, RSE::Dir direction, int param) =0 ;		// type is ROTATE or SELECT
};
...
public:
	...
	RotarySelector(int portDpinA, int portDpinB, int portDselectorPin, RotaryAction *action, int param , int max);
	~RotarySelector(){}
